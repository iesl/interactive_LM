...
def load_w_ind(f_in, max_sent_num, idx2word_freq, min_freq, stop_ind_set):
    w_ind_gpt2_corpus = []
    w_ind_spacy_corpus = []
    idx_gpt2_to_spacy = []
    #last_sent = ''
...
with open(corpus_input_name) as f_in:
    w_ind_gpt2_corpus, w_ind_spacy_corpus_new, idx_gpt2_to_spacy, idx2word_freq_new = load_w_ind(f_in, args.max_sent_num, idx2word_freq, args.min_freq, stop_ind_set)
...
def store_tensors(f_out,w_ind_gpt2_corpus, w_ind_spacy_corpus_new, idx_gpt2_to_spacy, start_idx, end_idx):
    ...
    torch.save([w_ind_gpt2_tensor, w_ind_spacy_tensor, idx_gpt2_to_spacy_tensor],f_out)
